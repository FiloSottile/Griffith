...
class Plugin(movie.Movie):
	...
	def original_title(self):
		self.original_title = gutils.trim(self.page, '<span class="styt">', '</span>')
		tmp = len(self.original_title) - 1
		if self.original_title[tmp:] == ' ':
			self.original_title = self.original_title[:tmp]
		self.original_title = string.replace(self.original_title, "\t", '')
		self.original_title = string.replace(self.original_title, "\n", '')
		if string.find(self.original_title, '(') > -1:
			self.original_title = gutils.before(self.original_title, '(')
