...
class Plugin(movie.Movie):
	...
	def get_director(self):
		...
		self.director = gutils.strip_tags(gutils.trim(self.page_unicode, u'<div><b>Realização:</b><br>', u'</div>')).encode(self.encode)
	...
	def get_runtime(self):
		...
		self.runtime = gutils.trim(self.page_unicode, u'<td><b>Duração:</b> ', u' min.</td>').encode(self.encode)

	def get_genre(self):
		"""Finds the film's genre"""
		self.genre = gutils.trim(self.page_unicode, u'<b>Género:</b><br>', u'</div>').encode(self.encode)
	...
	def get_o_site(self):
		...
		self.o_site = gutils.trim(self.page_unicode, \
				...
				u'" title="Consultar título no Internet Movie Data Base"').encode(self.encode)
	...
	def get_country(self):
		...
		self.country = gutils.trim(self.page_unicode, u'<b>País:</b><br>', u'</div>').encode(self.encode)
